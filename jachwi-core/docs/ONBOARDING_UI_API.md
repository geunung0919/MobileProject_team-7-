...
`GET /api/v1/onboarding/form`을 기존 Bearer 토큰으로 호출합니다.
`steps`는 자취 상태, 요리 환경, 예산·지역, 초기 재고의 4단계이며 확인/결과 화면은 포함하지 않습니다.
각 fields 항목의 name은 저장 JSON 필드, label은 질문, widget은 입력 방식입니다.
option_labels의 키를 전송 값으로, 값을 표시 문구로 사용합니다.
skip_value는 건너뛰기 시 전송할 값입니다. 조리 도구의 empty_label 선택은 []로 전송하고 다른 도구 선택을 해제합니다.
정수 범위, 필수 항목, enum, 지역 코드 패턴은 input_schema에서 읽습니다. 이 스키마는 실제 서버 검증 모델에서 생성됩니다.
프론트엔드 검증과 별개로 서버는 모든 저장 입력을 다시 검증합니다.
지역 검색 제공자나 검색 API는 이 응답에 포함되지 않으므로 연결 전에는 건너뛰기를 제공합니다.
form_version은 질문 표시 계약 버전이며 진단 rule_version과 별개입니다.
조회는 사용자 프로필을 생성하거나 수정하지 않습니다. 폼 조회 실패 시 다시 시도할 수 있게 안내합니다.
